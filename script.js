...
async function runKnapsack() {
  ...
  const items = kpItems.map(it => ({name: it.label, w: it.w, v: it.v}));
...
async function runBFS() {
  ...
  // Dynamically find the deepest connected leaf node to start BFS from
  const connectedIds = new Set();
  g.edges.forEach(([u, v]) => { connectedIds.add(u); connectedIds.add(v); });
  const startNode = [...connectedIds].sort((a, b) => b - a)[0] || g.nodes[g.nodes.length - 1].id;

  log('bfs-log', `Starting BFS from ${g.nodes.find(n => n.id === startNode)?.label || 'node ' + startNode} (node ${startNode})`, 'info');
...
function generateFloydWarshall() {
  ...
  // Generate random directed graph with guaranteed connectivity
  fwDist = Array(n).fill(null).map((_, i) =>
    Array(n).fill(null).map((_, j) => {
      if (i === j) return 0;
      if (Math.random() > 0.55) return Math.floor(Math.random() * 9) + 1;
      return Infinity;
    })
  );
  // Ensure at least one path chain A→B→C→D→E
  for (let i = 0; i < n - 1; i++) {
    if (fwDist[i][i + 1] === Infinity) {
      fwDist[i][i + 1] = Math.floor(Math.random() * 7) + 1;
    }
  }
  resetFloydWarshall();
}
  ...
  return `
    <div class="viz-area">
      ...
      <div style="margin:16px;font-size:13px">
        <div style="margin-bottom:8px"><strong>Suspect A Log:</strong> <span style="color:var(--accent)">${lcsSeq1}</span></div>
        <div style="margin-bottom:16px"><strong>Suspect B Log:</strong> <span style="color:var(--accent3)">${lcsSeq2}</span></div>
      </div>
...
function generateLCS() {
  const words = ['SEARCH','TRACK','FOLLOW','FIND','ARREST','WATCH','FLEE','HIDE','STEAL','HACK',
    'BANK','VAULT','SAFE','CASH','JEWEL','DATA','CODE','FILE','DRUG','ARMS'];
  const pick = () => words[Math.floor(Math.random() * words.length)];
  const makeSeq = () => Array.from({length: Math.floor(Math.random() * 3) + 3}, pick).join('');
  lcsSeq1 = makeSeq();
  lcsSeq2 = makeSeq();
  renderCase(12);
}
...
function buildTopoSort() {
  return `
    <div class="viz-area">
      ...
      <div id="topo-graph" style="text-align:center;margin:20px 0;font-size:12px"></div>
...
// Dynamic DAG for investigation tasks
let topoTasks = {};

const topoTaskPool = [
  'Interview', 'Gather', 'Check', 'Analyze', 'Verify',
  'Report', 'Arrest', 'Confirm', 'Search', 'Trace',
  'Decode', 'Survey', 'Detain', 'Inspect', 'Brief'
];

function generateRandomDAG() {
  const shuffled = [...topoTaskPool].sort(() => Math.random() - 0.5);
  const count = Math.floor(Math.random() * 4) + 6;
  const tasks = shuffled.slice(0, count);
  tasks.push('Close');

  const dag = {};
  tasks.forEach(t => dag[t] = []);

  // Each task connects to 1-2 tasks that appear later in the list (ensures DAG)
  for (let i = 0; i < tasks.length - 1; i++) {
    const later = tasks.slice(i + 1);
    const edgeCount = Math.min(later.length, Math.floor(Math.random() * 2) + 1);
    const picks = [...later].sort(() => Math.random() - 0.5).slice(0, edgeCount);
    dag[tasks[i]] = picks;
  }

  // Ensure all non-Close tasks lead somewhere
  for (const task of tasks) {
    if (task !== 'Close' && dag[task].length === 0) {
      dag[task] = ['Close'];
    }
  }
  return dag;
...
function renderTopoGraph() {
  const graphDiv = document.getElementById('topo-graph');
  if (!graphDiv) return;
  const colors = ['var(--accent)', 'var(--accent3)', 'var(--green)', 'var(--accent2)'];
  let html = '';
  let colorIdx = 0;
  for (const [task, deps] of Object.entries(topoTasks)) {
    if (deps.length > 0) {
      html += `<div style="color:${colors[colorIdx % colors.length]};margin:6px">${task} → ${deps.join(', ')}</div>`;
      colorIdx++;
    }
  }
...
async function runTopoSort() {
  ...
  if (Object.keys(topoTasks).length === 0) {
    topoTasks = generateRandomDAG();
    renderTopoGraph();
  }
  ...
  while (queue.length > 0) {
    ...
    result.push(task);
    const successors = topoTasks[task] || [];
    log('topo-log', `Selected: "${task}" — can now schedule ${successors.join(', ') || 'none'}`, 'warning');
...
function initBSData() {
  ...
  while (values.size < 20) {
    values.add(Math.floor(Math.random() * 200) + 5);
  }
  bsWitnesses = [...values].sort((a, b) => a - b);
}

function buildBinarySearch() {
  if (bsWitnesses.length === 0) initBSData();
  ...
  return `
    <div class="viz-area">
      ...
      <div id="bs-list" style="margin:16px;font-size:12px;display:grid;grid-template-columns:repeat(5,1fr);gap:8px">
        ${bsWitnesses.map((t,i) => `<div class="item-card" id="bs-item-${i}" style="padding:8px;cursor:pointer" onclick="searchWitness(${i})">Witness-${String(i+1).padStart(2,'0')}<br/>Time: ${t}h</div>`).join('')}
      </div>
    </div>
    <div style="margin:16px">
      <label>Search for time:</label><br/>
      <input type="number" id="bs-target" min="0" max="999" value="${bsWitnesses[Math.floor(bsWitnesses.length/2)]}" style="padding:6px;margin:8px 0"/>
...
async function runBinarySearch() {
  ...
  log('bs-log', `Searching for witness with interrogation time: ${target}h in ${bsWitnesses.length} records`, 'info');
  
  let left = 0, right = bsWitnesses.length - 1, comparisons = 0, found = false;
  ...
  while (left <= right) {
    ...
    comparisons++;
    log('bs-log', `Checking index ${mid}: time=${bsWitnesses[mid]}h vs target=${target}h`, 'warning');
    
    await sleep(state.speed);
    if (midEl) midEl.classList.remove('active');
    
    if (bsWitnesses[mid] === target) {
      ...
      found = true;
      break;
    } else if (bsWitnesses[mid] < target) {
      left = mid + 1;
    } else {
      right = mid - 1;
    }
...
function searchWitness(idx) {
  document.getElementById('bs-target').value = bsWitnesses[idx];
  runBinarySearch();
}
...
function generateRabinKarp() {
  const words = ['CRIME','SCENE','EVIDENCE','SUSPECT','WITNESS','ARREST','PATROL','DETECTIVE',
    'FORENSIC','MYSTERY','CLUE','MOTIVE','ALIBI','PURSUIT','WARRANT','HEIST','CIPHER',
    'DECODE','ESCAPE','AMBUSH','TARGET','AGENT','SHADOW','TRACE','COVERT','BREACH'];
  // Build random text from random words
  const count = Math.floor(Math.random() * 5) + 6;
  const selected = [];
  for (let i = 0; i < count; i++) {
    selected.push(words[Math.floor(Math.random() * words.length)]);
  }
  rkText = selected.join('');
  // Pick a pattern that exists in the text (substring from a used word)
  const srcWord = selected[Math.floor(Math.random() * selected.length)];
  const patLen = Math.min(srcWord.length, Math.floor(Math.random() * 3) + 3);
  const patStart = Math.floor(Math.random() * Math.max(1, srcWord.length - patLen + 1));
  rkPattern = srcWord.substring(patStart, patStart + patLen);
